...
async def task_starter(message, text):
    ...
    # Always allow new task initiation - the queue system handles execution order
...
async def execute_queued_task(task):
    """Execute a task from the queue system"""
    global BOT, MSG, BotTimes, Messages, Paths, Transfer, TaskError
    ...
    # Apply task settings to global BOT state
    BOT.SOURCE = task.source
    BOT.Mode.mode = task.mode
    BOT.Mode.type = task.task_type
    BOT.Mode.ytdl = task.ytdl
    BOT.Options.custom_name = task.custom_name
    BOT.Options.zip_pswd = task.zip_pswd
    BOT.Options.unzip_pswd = task.unzip_pswd
    BOT.Options.stream_upload = task.stream_upload
    BOT.Options.caption = task.caption_style
    BOT.Options.convert_video = task.convert_video
    BOT.Options.video_out = task.video_out
    ...
    MSG.status_msg = await colab_bot.send_message(
        chat_id=OWNER,
        text=f"#QUEUED_TASK_STARTED\n\n**Task ID:** `{task.task_id}`\n**Mode:** {task.mode.upper()}\n**Starting your task...🦐**",
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("Cancel ❌", callback_data=f"cancel_queue_{task.task_id}")],
            ]
        ),
    )
    task.status_msg = MSG.status_msg
    
    try:
        # Execute the task scheduler
        await taskScheduler()
    except Exception as e:
        logging.error(f"Task {task.task_id} execution error: {e}")
        raise
    finally:
